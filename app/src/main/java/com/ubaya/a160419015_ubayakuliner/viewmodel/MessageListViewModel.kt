...
class MessageListViewModel(application: Application) : AndroidViewModel(application) {
    val messageLiveData = MutableLiveData<ArrayList<Message>>()
    ...
    fun refresh() {
        ...
        val url = "http://192.168.18.43/anmp/Project_UTS/message.php"
        val stringRequest = StringRequest(
            Request.Method.GET, url,
            {
                val sType = object : TypeToken<ArrayList<Message>>() {}.type
                val result = Gson().fromJson<ArrayList<Message>>(it, sType)
